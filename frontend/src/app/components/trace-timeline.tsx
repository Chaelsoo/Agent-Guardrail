...
function formatSpanName(name: string): string {
  // "tool_pre:bash" → "tool · bash (pre)"
  // "tool_post:bash" → "tool · bash (post)"
  // "prellm_policy" → "prellm policy"
      ...
      {traces.map((trace) => (
        <div
          ...
          className={`bg-[#12171f] border rounded p-4 transition-colors ${
            !trace.finalized ? 'border-[#3b82f6]/40' : 'border-[#1e293b]'
          }`}
        >
          {/* Header */}
          ...
          {trace.steps.length > 0 && (
            <div className="space-y-2">
              ...
              {trace.steps.map((step, index) => (
                <div
                  key={step.id}
                  className={`flex items-start gap-3 ${step.isTool ? 'ml-4' : ''}`}
                >
                  {/* Timeline connector */}
                  <div className="flex flex-col items-center pt-1">
                    {step.isTool ? (
                      <Wrench
                        ...
                        style={{
                          color: step.status === 'blocked' ? '#ef4444' :
                                 step.status === 'redacted' ? '#f59e0b' :
                                 step.status === 'flagged' ? '#eab308' : '#6366f1',
                        }}
                      />
                    ) : (
                      <div
                    ...
                    {index < trace.steps.length - 1 && (
                      <div className="w-px h-8 bg-[#1e293b] mt-1" />
                    )}
                  </div>
                  ...
                  <div className="flex-1 pb-2">
                    <div className="flex items-center justify-between mb-1">
                      <span className={`text-[12px] ${step.isTool ? 'text-[#a5b4fc]' : 'text-[#e4e7eb]'}`}>
